          ...
          <table class="table table-bordered table-striped table-sm">
            <thead>
              <tr>
                ...
                <th scope="col" class="text-left mw-130" width="10%">{{ $t('calculationSheetType')}}</th>
            ...
            <tbody>
              <tr v-if="awardingSheets.length" v-for="awarding in awardingSheets">
                ...
                <td class="text-left">{{getCalculationSheetTypeName(awarding.calculationSheetType)}}</td>
                ...
                <td class="action-column text-center">
                  <router-link
                    ...
                    title="Action"
                    class="text-primary"
                   :to="{ name: 'viewAwardingSheet', params: {CalSheetId: awarding.id, CalSheetType: awarding.calculationSheetType}}">
                    <i class="fa fa-eye"></i>
                  </router-link>
                  <!-- <router-link
                    ...
                    :title="$t('Edit')"
                    class="text-primary"
                    :to="{ name: 'NewAwardingSheet', params: {awardingID: awarding.id}}">
                  ...
                  <a
                    ...
                    :title="$t('Delete')"
                    class="text-danger"
                    v-on:click="deleteAwardingSheet(awarding.id,awarding.calculationSheetType);"
                  >
...
export default {
  ...
  methods: {
    ...
    Excel() {
      ...
      var urlStr =
        ...
        "api/AwardingSheet/ExportToExcel?PageIndex=" +
    ...
    getAwardingSheetList() {
      ...
      var urlStr =
        this.$store.getters.serverURI +
        "api/AwardingSheet/GetCalculationSheets?PageIndex=" +
        this.page.currentPage +
        "&PageSize=" +
        this.page.itemsPerPage +
        "&RowsCount=&awardingSheetId=" +
    ...
    deleteAwardingSheet(awardingSheetID, calculationSheetType) {
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            if(calculationSheetType ==1){
              urlStr =this.$store.getters.serverURI +
              "api/CalculationSheets/" + awardingSheetID+"/UpdateStatusToApproved";
            }else if (calculationSheetType ==2){
              urlStr =this.$store.getters.serverURI +
              "api/TenderCalculationSheets/" + awardingSheetID+"/UpdateStatusToApproved";
            }
            else if (calculationSheetType==3){
              urlStr =this.$store.getters.serverURI +
              "api/LabCalculationSheets/" + awardingSheetID+"/UpdateStatusToApproved";
            }
    ...
    getCalculationSheetRefernce() {
      var urlStr =
        this.$store.getters.serverURI +
        "api/CalculationSheets/getCalculationSheetsTinyModelOfAwardingSheets";
    ...
    getQuoteNumbers() {
      var urlStr =
        this.$store.getters.serverURI +
        "api/PriceQuotation/getPriceQuotationsTinyModelOfAwardingSheets";
  ...
  created: function() {
    this.getCalculationSheetRefernce();
